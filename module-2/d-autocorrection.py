...
class Trie:
    class Node:
        ...
        def add_child(self, substr: str):
            ...
            Временная сложность - O(len(substr)).
            '''
        ...
        def split(self, i: int, substr: str):
            ...
            Временная сложность O(max(k, len(substr))), где k - начальная длина подстроки (перехода) текущей вершины.
            '''
    ...
    def add_word(self, word: str):
        ...
        Сложность алгоритма - O(max(n, len(w))), где n - длина добавляемого слова word, w - самое длинная подстрока (переход) во всём дереве.
        '''
        if not word:
            return
