...
class Trie:
    ...
    def suggest(self, word: str) -> (ResultType, list):
        ...
        текущий путь уже не сможет иметь ошибки в дальнейшем => на каждом ярусе из каждой из m-1 вершин существует
        ...
        (Т) на каждом ярусе из каждой m-1 вершины существует максимум 1 допустимый путь до листа. Его длина также равна
        h-i, где i - номер текущего яруса (см. п. 3).
