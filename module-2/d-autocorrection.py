...
class Trie:
    ...
    def suggest(self, word: str) -> (ResultType, list):
        ...
        так и остальные m-1 вершин, так как счетчик ошибок для слова word остается равным 0 на протяжении всего
        обхода дерева (т.к. ошибка - лишний символ, она появляется в конце). Счетчик ошибок увеличивается на 1 при
        переходе в каждую из m-1 вершин.
        ...
        ∑(1+(m-1)(h-i)) = ∑(1+(m-1)(n-i)) = n+∑(mn-n-im+i) = n+mn^2-n^2-m∑i+∑i = n^2(m-1)+n+(1-m)∑i =
        = n^2(m-1)+n+(1-m)((0+n)/2*n) = n^2(m-1)+n+(1-m)(n^2/2) = n^2(m-1)+n+n^2/2-mn^2/2 = n^2((m-1)+1/2-m/2)+n =
        = n^2(m-1)/2+n = O(mn^2)
