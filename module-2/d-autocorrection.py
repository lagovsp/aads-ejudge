...
class Trie:
    ...
    def suggest(self, word: str) -> (ResultType, list):
        ...
        3. Пусть i - счетчик пройденных ярусов дерева. Идем от корня, i = 0.
        ...
        вершин дерева, теперь будет определяться O(n).

        6. В случае, если при переходе в какую-то из m-1 вершин мы проверяем одну из ошибок (В), (З) или (У), то
        текущий пути не может иметь ошибки в дальнейшем пути => существует максимум 1 путь до листа, причем его длина
        будет равна h-i для i-ого яруса сверху.
        ...
        (Т) у нас также существует максимум 1 путь до листа. Его длина также равна h-i для i-ого яруса сверху.
